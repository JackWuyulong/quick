...
<body ng-app="myapp" ng-controller="mycontroller">
    ...
    <div class="panel">
        ...
        <div class="login_input">
            <span>密码</span>
            <input  type="password" placeholder="请输入密码" ng-model="pwd"/>
        </div>
        <div ng-click="sub()" id="btn"  style="background-color:#3C74F2;width:306px;height:32px;line-height:32px;text-align: center;color:#fff;border-radius:4px;margin-top:30px;margin-left:60px;">
            登录
        </div>
    </div>
...
<script>
  	...
  	app.controller("mycontroller",function($scope,$http){
  		...
  		$scope.sub = function(){
  			var url="__APP__/Index/checkManage";
  			$http({
  				url:url,
  				method:"post",
  				headers:{
  				...
  				if(data.status==1){
  					window.location.href = "__APP__/Index/edit"
  				}else{
  					alert(data.info)
  				}
  			})
